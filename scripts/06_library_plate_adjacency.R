...
output_dir <- file.path("output", "TODO-06")
...
library_key <- 
  ...
  transmute(
    ...
    plateid = `Library Plate`,
    plate_row = `Library plate row`,
    plate_col = `Library plate col`,
...
readr::write_csv(
  plate_metrics,
  file.path(output_dir, "library_plate_adjacency_metrics.csv")
)
readr::write_csv(
  plate_permutation_summary,
  file.path(output_dir, "library_plate_permutation_summary.csv")
)
readr::write_csv(
  plate_permutation_distributions,
  file.path(output_dir, "library_plate_permutation_distributions.csv")
)
readr::write_csv(
  plate_diagnostics,
  file.path(output_dir, "library_plate_diagnostics.csv")
)
readr::write_csv(
  plate_counts,
  file.path(output_dir, "library_plate_counts.csv")
)
...
admixedness_heatmap <- 
  ...
  labs(
    title = "Library Plate Admixture Heatmap",
...
ggsave(
  filename = file.path(output_dir, "library_plate_admixedness_heatmap.png"),
...
k4_heatmap <- 
  ...
  labs(
    title = "Library Plate K4 Affiliation Heatmap",
    subtitle = "Tile layout mirrors plate coordinates (rows, columns)",
    x = "Plate column",
    y = "Plate row",
    fill = "K4 proportion"
...
ggsave(
  filename = file.path(output_dir, "library_plate_k4_heatmap.png"),
  plot = k4_heatmap,
  width = 12,
  height = 8,
  units = "in",
...
cluster_heatmap <- 
  ...
  labs(
    title = "Library Plate Dominant Cluster Heatmap",
...
ggsave(
  filename = file.path(output_dir, "library_plate_cluster_heatmap.png"),
...
admixedness_boxplot <- 
  ...
  coord_flip() +
  labs(
    title = "Admixedness by Library Plate",
    x = "Plate",
    y = "Admixedness"
  ) +
  theme_minimal() +
...
ggsave(
  filename = file.path(output_dir, "library_plate_admixedness_boxplot.png"),
