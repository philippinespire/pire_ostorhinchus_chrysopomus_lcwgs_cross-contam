...
extractions_key <- 
  ...
  transmute(
    ...
    date_extracting,
    tube_stuffer,
    date_extracting_date = as.Date(stringr::str_sub(date_extracting, 1, 10)),
...
extraction_grouped <- 
  extraction_joined %>%
  arrange(date_extracting_date, tube_stuffer, individual_num, individual_id) %>%
  group_by(date_extracting_date, tube_stuffer)
...
results <- purrr::map2(
  ...
  function(group_df, index) {
    ...
    group_id <- paste(
      coalesce(as.character(key_row$date_extracting_date), "NA"),
      coalesce(key_row$tube_stuffer, "NA"),
      sep = "|"
    )
    ...
    observed_row <- 
      observed_metrics %>%
      mutate(
        date_extracting_date = key_row$date_extracting_date,
        tube_stuffer = key_row$tube_stuffer,
        extraction_group = group_id,
...
#### AGGREGATE OUTPUTS ####
extraction_adjacency_metrics <- purrr::map_dfr(results, "metrics")
extraction_permutation_summary <- purrr::map_dfr(results, "perm_summary")
extraction_permutation_distributions <- purrr::map_dfr(results, "perm_dist")

#### EXPORT OUTPUTS ####
readr::write_csv(
  extraction_adjacency_metrics,
  file.path(output_dir, "extraction_order_adjacency_metrics.csv")
)
readr::write_csv(
  extraction_permutation_summary,
  file.path(output_dir, "extraction_order_permutation_summary.csv")
)
readr::write_csv(
  extraction_permutation_distributions,
  file.path(output_dir, "extraction_order_permutation_distributions.csv")
)
