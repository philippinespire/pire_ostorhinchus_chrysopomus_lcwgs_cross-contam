...
Goal: Test dilution plate adjacency effects using `Dilution Plate`, `Dilution Plate Row`, `Dilution Plate Col` to see whether admixed wells cluster near wells from different genetic groups.
...
3. Standardize dilution plate coordinates (row letter to index, column to integer) and create a `plate_key` (e.g., `Dilution Plate`).
4. Flag missing/duplicate wells; preserve all records with diagnostics rather than dropping.
5. For each dilution plate, compute adjacency metrics based on 4-neighbor wells:
...
## Outputs
- `output/dilution_plate_adjacency_metrics.csv`
- `output/dilution_plate_permutation_summary.csv`
- `output/dilution_plate_permutation_distributions.csv`
- `output/dilution_plate_diagnostics.csv`
- `output/dilution_plate_counts.csv`
- `output/dilution_plate_admixedness_heatmap.png`
- `output/dilution_plate_k4_heatmap.png`
- `output/dilution_plate_cluster_heatmap.png`
- `output/dilution_plate_admixedness_boxplot.png`
